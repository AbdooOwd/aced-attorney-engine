...
const _SPEAKER_TEXTURE_SCALER: f32 = 4.0;
...
pub async fn update_speaker_emotion_texture(emotion: SpeakerEmotion, speaker_tex: &Texture2D) {
    ...
    let new_image = get_image(emotion_texture_path.as_str()).await;
    speaker_tex.update(&new_image);
    draw_from_texture(speaker_tex, 50.0, screen_height() - f32::from(new_image.height) * TEXTURE_SCALING, TEXTURE_SCALING);
}